...
public interface IncentiveContractSalesCancellationRepository
	...
	@Query(value = "SELECT * FROM INC_DLR_CONTRACT_SALES_CANCELLATION i WHERE i.CONTRACT_STATUS = 'P' AND i.CONTRACT_REGISTRATION_DATE BETWEEN :effectiveDate AND :expiryDate AND i.CONTRACT_CANCELLATION_DATE BETWEEN :effectiveDate AND :expiryDate AND i.DEALER_CODE =:dealerCode", nativeQuery = true)
