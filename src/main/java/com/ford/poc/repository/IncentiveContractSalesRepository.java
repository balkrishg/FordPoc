...
public interface IncentiveContractSalesRepository extends JpaRepository<IncentiveContractSales, Long> {
	...
	@Query(value = "SELECT * FROM INC_DLR_CONTRACT_SALES i WHERE i.CONTRACT_STATUS IN ('A','I','L','P') AND i.CONTRACT_REGISTRATION_DATE BETWEEN :effectiveDate AND :expiryDate AND i.DEALER_CODE =:dealerCode", nativeQuery = true)
